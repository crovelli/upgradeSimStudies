#define tree_reader_up_cxx
#include "tree_reader_up.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void tree_reader_up::Loop()
{
  if (fChain == 0) return;
...
#ifdef tree_reader_up_cxx
tree_reader_up::tree_reader_up(TTree *tree) : fChain(0)
{
  if (tree == 0) {
    TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("myFile.root");
    if (!f || !f->IsOpen()) {
      f = new TFile("myFile.root");
    }
    TDirectory * dir = (TDirectory*)f->Get("myFile.root/myanalysis");
    dir->GetObject("pippo",tree);
  }
  Init(tree);
}

tree_reader_up::~tree_reader_up()
{
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t tree_reader_up::GetEntry(Long64_t entry)
{
...
Long64_t tree_reader_up::LoadTree(Long64_t entry)
{
...
void tree_reader_up::Init(TTree *tree)
{
   // Set object pointer
   HLTNames = 0;
   HLTResults = 0;
...
Bool_t tree_reader_up::Notify()
{
  return kTRUE;
}

void tree_reader_up::Show(Long64_t entry)
{
  if (!fChain) return;
  fChain->Show(entry);
}

Int_t tree_reader_up::Cut(Long64_t entry)
{
  return 1;
}
#endif // #ifdef tree_reader_up_cxx
